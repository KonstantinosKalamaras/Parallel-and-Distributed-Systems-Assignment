...
int main(int argc, char *argv[]){
    ...
    int dif,starti, startj, endi, endj, row, col ; 
    clock_gettime(CLOCK_MONOTONIC, &tStart);
    
    /*only calculate square for non zeros of matrix(skips Hadamard)*/
    /*we pick the second for to parallelize*/
    #pragma omp parallel private(row,col,starti,startj,endi,endj,dif)
    {
